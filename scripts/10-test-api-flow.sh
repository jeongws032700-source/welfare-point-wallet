...
echo "4. 포인트 충전"
request_json POST "$BASE/api/bank/deposit" '{"amount":10000,"memo":"테스트 충전"}' "$TOKEN"

echo "5. 포인트 사용"
request_json POST "$BASE/api/bank/withdraw" '{"amount":5000,"memo":"테스트 사용"}' "$TOKEN"

echo "6. 단일 포인트 선물"
request_json POST "$BASE/api/bank/transfer" '{"toAccountNumber":"110-100-000002","amount":3000,"memo":"테스트 선물"}' "$TOKEN"

echo "7. 다중 포인트 선물"
request_json POST "$BASE/api/bank/multi-transfer" '{"memo":"다중선물","targets":[{"toAccountNumber":"110-100-000002","amount":1000},{"toAccountNumber":"110-100-000003","amount":1000}]}' "$TOKEN"
