...
function txLabel(type) {
  return { DEPOSIT: "충전", WITHDRAW: "사용", TRANSFER_OUT: "선물", TRANSFER_IN: "선물받음" }[type] || type;
}
    ...
    return (
      <main className="phone-bg">
        <section className="phone-card auth-card">
          ...
          <p className="eyebrow">COMPANY WELFARE POINT</p>
          <h1>사내 복지포인트<br />지갑 서비스</h1>
          <p className="lead">조회는 모바일 조회 서버, 사용/선물은 별도 포인트 이용 서버에서 처리됩니다. 로그인 세션은 Redis에 저장됩니다.</p>
    ...
    <main className="phone-bg">
      <section className="phone-frame">
        ...
        <section className="balance-card">
          <div className="card-top"><span>메인 포인트 지갑</span><b>{account.data?.status || "-"}</b></div>
          <h2>{account.isLoading ? "조회 중" : money(account.data?.balance)}</h2>
          <p>{account.data?.accountNumber || "지갑 정보를 불러오는 중입니다"}</p>
          <div className="quick-actions">
            <a href="/action">선물</a><a href="/action">충전</a><a href="/action">사용</a>
          </div>
        </section>
        ...
        <section className="section-block">
          <div className="section-head"><h2>최근 선물 대상</h2><a href="/action">관리</a></div>
        ...
        <section className="section-block">
          <div className="section-head"><h2>포인트 이용내역</h2><button onClick={() => { account.refetch(); tx.refetch(); recent.refetch(); }}>새로고침</button></div>
          {latest && <div className="latest-card"><span>최근 이용</span><strong>{txLabel(latest.type)}</strong><b>{money(latest.amount)}</b><p>{latest.memo || "메모 없음"}</p></div>}
          <div className="tx-list">
            ...
            {!transactions.length && <p className="empty">포인트 이용내역이 없습니다.</p>}
          </div>
        </section>

        <nav className="bottom-nav"><span className="active">홈</span><a href="/action">이용</a><span>내역</span><span>설정</span></nav>
      </section>
    </main>
  );
}
