    ...
    return (
      <main className="auth-shell">
        <section className="auth-card">
          ...
          <p className="auth-copy">회원, 포인트 지갑, 이용 내역, Redis 감사 로그를 관리하는 PC 전용 운영 화면입니다.</p>
    ...
    <div className="admin-layout">
      <aside className="sidebar">
        ...
        <nav className="side-nav">
          ...
          <NavItem icon="지갑" label="지갑 관리" />
          <NavItem icon="이용" label="포인트 이용 모니터링" />
          <NavItem icon="로그" label="Redis 감사 로그" />
        </nav>
        <div className="side-footer">
          <span>복지포인트 운영 3001</span>
          <small>GCP VM · Nginx 라우팅</small>
        </div>
      </aside>
      ...
      <div className="workspace">
        <header className="topbar">
          <div>
            <p className="crumb">복지포인트 운영 / 운영자 콘솔</p>
            <h1>복지포인트 운영 대시보드</h1>
          </div>
        ...
        <main className="content">
          <section className="hero-card">
            <div>
              <span className="hero-label">사내 복지포인트</span>
              <h2>사내 복지포인트 지갑 현황을 한 화면에서 관리합니다</h2>
          ...
          <section className="metric-grid">
            ...
            <div className="metric-card"><span>지갑</span><strong>{data.accountCount ?? accounts.length}</strong><p>생성·해지 지갑</p></div>
            <div className="metric-card"><span>포인트 이용</span><strong>{data.transactionCount ?? transactions.length}</strong><p>최근 이용 내역 모니터링</p></div>
            <div className="metric-card"><span>총 포인트</span><strong>{formatMoney(totalBalance)}</strong><p>대시보드 캐시 대상</p></div>
          </section>
          ...
          <section className="operation-grid">
            ...
            <div className="panel">
              <div className="panel-head"><h2>지갑 생성</h2><span>관리자 발급</span></div>
              <input placeholder="사용자 ID" value={account.userId} onChange={(e) => setAccount({ ...account, userId: Number(e.target.value) })} />
              <input placeholder="초기 포인트" value={account.initialBalance} onChange={(e) => setAccount({ ...account, initialBalance: Number(e.target.value) })} />
              <button onClick={() => createAccount.mutate(account)}>지갑 생성</button>
            ...
            <div className="panel danger-panel">
              <div className="panel-head"><h2>상태/지갑 해지</h2><span>위험 작업</span></div>
              ...
              <input placeholder="해지 지갑 ID" value={closeAccount.accountId} onChange={(e) => setCloseAccount({ accountId: e.target.value })} />
              <button className="danger" onClick={() => closeAccountMutation.mutate({})}>지갑 해지</button>
            </div>
          </section>
          ...
          <section className="data-grid">
            ...
            <div className="table-panel wide">
              <div className="panel-head"><h2>지갑 목록</h2><span>{accounts.length}건</span></div>
              <div className="table-wrap">
                <table><thead><tr><th>ID</th><th>User</th><th>지갑 번호</th><th>포인트</th><th>상태</th></tr></thead><tbody>{accounts.map((a) => <tr key={a.id}><td>{a.id}</td><td>{a.userId}</td><td>{a.accountNumber}</td><td>{formatMoney(a.balance)}</td><td><span className={`status ${String(a.status).toLowerCase()}`}>{String(a.status)}</span></td></tr>)}</tbody></table>
              </div>
            </div>

            <div className="table-panel">
              <div className="panel-head"><h2>최근 포인트 이용</h2><span>{transactions.length}건</span></div>
              <div className="timeline">{transactions.slice(0, 8).map((t) => <div className="timeline-item" key={t.id}><div><strong>{txLabel(t.type)}</strong><p>{t.memo || "-"} · {t.fromAccountNumber || "외부"} → {t.toAccountNumber || "-"}</p></div><b>{formatMoney(t.amount)}</b></div>)}</div>
            </div>
