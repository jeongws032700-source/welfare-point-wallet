...
export default function MobileActionPage() {
  ...
  const [memo, setMemo] = useState("모바일 선물");
      ...
      <main className="mobile-shell action-shell">
        <section className="device-card login-card">
          ...
          <section className="signin-hero">
            ...
            <p className="eyebrow">POINT ACTION SERVER</p>
            <h2>충전·사용·선물은<br />로그인 후 진행합니다</h2>
            <p>
    ...
    <main className="mobile-shell action-shell">
      <section className="device-card">
        ...
        <section className="account-card google-blue-card">
          ...
          <h2>{account.data ? money(account.data.balance) : "포인트 조회 중"}</h2>
          <p>{account.data?.accountNumber || "지갑 정보를 불러오고 있습니다"}</p>
        </section>
        ...
        <section className="form-card transfer-card">
          ...
          <div className="action-buttons">
            <button className="secondary-btn" disabled={isBusy} onClick={() => deposit.mutate({ amount: Number(amount), memo: memo || "충전" })}>충전</button>
            <button className="secondary-btn" disabled={isBusy} onClick={() => withdraw.mutate({ amount: Number(amount), memo: memo || "사용" })}>사용</button>
            <button className="primary-btn span2" disabled={isBusy} onClick={() => transfer.mutate({ toAccountNumber: to, amount: Number(amount), memo: memo || "선물" })}>선물 실행</button>
            <button className="dark-btn span2" disabled={isBusy} onClick={() => multi.mutate({ memo: "다중선물", targets: [{ toAccountNumber: "110-100-000002", amount: 1000 }, { toAccountNumber: "110-100-000003", amount: 2000 }] })}>여러 회원에게 선물</button>
          </div>
        ...
        {result && (
          <section className="result-card">
            ...
            <p>현재 포인트 {money(result.account?.balance)}</p>
          </section>
        )}
            ...
            {(tx.data || []).slice(0, 6).map((item) => (
              <div className="tx-item" key={item.id}>
                <div>
                  ...
                  <strong>{item.memo || "포인트 이용"}</strong>
                  <p>{item.createdAt}</p>
                </div>
                <b>{money(item.amount)}</b>
              </div>
            ))}
            {!(tx.data || []).length && <p className="empty">포인트 이용내역이 없습니다.</p>}
          </div>
        </section>

        <nav className="bottom-nav">
          <a href="/">홈</a>
          <span className="active">이용</span>
          <a href="/">내역</a>
